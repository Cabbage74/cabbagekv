...
impl SsTableBuilder {
    ...
    pub fn add(&mut self, key: KeySlice, value: &[u8]) {
        ...
        let add_success = self.builder.add(key, value);
        assert!(
            add_success,
            "First key-value pair should always be added successfully even if it exceeds the block size"
        );
    ...
    pub fn build(
        ...
        if !self.builder.is_empty() {
            let encoded_block = self.builder.build().encode();
            self.meta.push(BlockMeta {
                offset: self.data.len(),
                first_key: KeyBytes::from_bytes(Bytes::from(std::mem::take(&mut self.first_key))),
                last_key: KeyBytes::from_bytes(Bytes::from(std::mem::take(&mut self.last_key))),
