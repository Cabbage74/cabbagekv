...
impl BlockMeta {
    ...
    pub fn encode_block_meta(
        ...
        // cabbage: the number of blockmetas
    ...
    pub fn decode_block_meta(mut buf: impl Buf) -> Vec<BlockMeta> {
        let num_elements = buf.get_u32() as usize;
        let mut block_meta = Vec::with_capacity(num_elements);
...
impl SsTable {
    ...
    pub fn open(id: usize, block_cache: Option<Arc<BlockCache>>, file: FileObject) -> Result<Self> {
        ...
        let bloom_data = file.read(bloom_offset, len - 4 - bloom_offset)?;
        let bloom = Bloom::decode(&bloom_data)?;

        let raw_meta = file.read(
            block_meta_offset as u64,
            bloom_offset - 4 - block_meta_offset as u64,
        )?;
    ...
    pub fn read_block(&self, block_idx: usize) -> Result<Arc<Block>> {
        ...
        let offset_end = if block_idx + 1 < self.block_meta.len() {
